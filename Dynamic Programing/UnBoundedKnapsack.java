...
//  humare pass cheezo kii unlimited quantity hee isme //
// same like knap sack 
...
public class UnBoundedKnapsack {
    public static void main(String[] args) {
        ...
        for(int i = 0;i<val.length;i++){
         ...
         int ans1 =  profit(val, weight, c,0);
    ...
    private static int profit( int[] val, int[] weight,int capacity,int i) {
       ...
       int drop = profit(val, weight, capacity, i); // ek bar pick kiya tho firse krr sakte hee isliye index agge nhii baddha 
       int pick =  val[i] + profit(val, weight, capacity-weight[i], i);
    ...
    private static int profit1( int[] val, int[] weight,int capacity,int i,int dp[][]) {
       ...
       int pick =  val[i] + profit1(val, weight, capacity-weight[i], i,dp); // ek bar pick kiya tho firse krr sakte hee isliye index agge nhii baddha 
